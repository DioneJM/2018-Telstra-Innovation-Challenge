...
void AzureIoT::setBoardCurrentTime(TelstraM1Device* IoTDevice){
  ...
  //Serial.println("time = " + String(num_yr) + " , " + String(num_mnth) + " , " + String(num_dy) + " , " + String(num_hr) + " , " + String(num_minu) + " , " + String(num_sec));
}
...
String AzureIoT::generateSas(char *key, String endPointUri)
{
 ...
 // Serial.println("sasExpirayTime is " );
 // Serial.println(sasExpiryTime);
  ...
  String hard_coded_sasKey = "SharedAccessSignature sr=hojinhub.azure-devices.net%2Fdevices%2Ftelstrakit&sig=2ARTTWfXbuZQ%2BazWW0HijDNl1PGACD6z0gDZEDvuSPY%3D&se=3417057476";
//  String hard_coded_sasKey = "SharedAccessSignature sr=hojinhub.azure-devices.net%2Fdevices%2Ftelstrakit&sig=7YvBHWzDVWCxkaEMhWaEwXZUZ3EedeYgBpSC9rAErsg%3D&se=1525512249";

  // SharedAccessSignature
//  return "Authorization: SharedAccessSignature sr=" + endPointUri + "&sig=" \
//         + urlEncode(encodedSign) + "&se=" + sasExpiryTime ;
    String output_string=String("Authorization: ")+hard_coded_sasKey;
